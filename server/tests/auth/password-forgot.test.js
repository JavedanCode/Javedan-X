...
describe('POST /auth/password/forgot', () => {
  ...
  it.skipIf(!env.EMAIL_ENABLED)('creates a reset token and sends a reset email', async () => {
  ...
  it('handles a password reset request for a nonexistent email', async () => {
    const response = await request(app).post('/auth/password/forgot').send({
  ...
  it.skipIf(!env.EMAIL_ENABLED)(
    'does not send another reset email during the cooldown period',
    async () => {
  ...
  it.skipIf(!env.EMAIL_ENABLED)(
    'replaces an old reset token after the cooldown expires',
    async () => {
      await request(app).post('/auth/password/forgot').send({
        email: user.email,
      });
